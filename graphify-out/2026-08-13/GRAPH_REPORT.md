...
## Corpus Check
- 102 files · ~799,207 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1246 nodes · 2610 edges · 87 communities (71 shown, 16 thin omitted)
...
6. `FlexCol` - 38 edges
7. `FlexRow` - 34 edges
8. `PlaceholderBox` - 34 edges
9. `Spacer` - 33 edges
10. `Div` - 32 edges

## Surprising Connections (you probably didn't know these)
- `test_field_makes_a_component_editable()` --calls--> `Text`  [EXTRACTED]
  tests/test_components.py → html_engine/components/text.py
- `test_document_add_coerces_and_names_itself()` --calls--> `Document`  [EXTRACTED]
  tests/test_components.py → html_engine/document.py
...
- `test_route_3_style_raw_escape_hatch()` --calls--> `Style`  [EXTRACTED]
  tests/test_monochrome.py → html_engine/styles.py
- `test_the_prose_note_names_the_target_language()` --calls--> `language_spec()`  [EXTRACTED]
  tests/test_translator.py → information_extraction/languages.py
...
### Community 0 - "run.py"
Cohesion: 0.07
Nodes (43): current_layout_path(), The layout that will actually be built — whatever ``ACTIVE`` names. This used…, _crop_to_page(), _page_metrics(), Path, render_png(), _collect_decision(), _collect_user_concerns() (+35 more)

### Community 1 - "test_components.py"
Cohesion: 0.10
Nodes (24): Div, Generic block container rendered as a ``<div>``. The simplest building block —…, corner_box(), Faint centred text behind the page content. Absolutely positioned and non-…, A placeholder pinned to one corner of the page — crest, QR block, stamp.…, Watermark, Tests for the components added for this project's documents, and for the…, Three properties separate a watermark from a heading: it sits behind, it never… (+16 more)
...
### Community 4 - "components/__init__.py"
Cohesion: 0.21
Nodes (6): html_engine.components — All renderable component types., Table components: Table, TableRow, TableCell. Supports both simple table…, A full ``<table>`` element. Can be built from: - Explicit ``TableRow`` objects…, A table row (``<tr>``) containing one or more cells. Parameters: cells:…, Table, TableRow

### Community 5 - "Document"
Cohesion: 0.11
Nodes (29): build_income_certificate(), _ea(), _lv(), Any, Income Certificate Layout — document_builder/income_certificate/layout_1.py…, build_pan(), build_pan(), Any (+21 more)

### Community 6 - "test_translator.py"
Cohesion: 0.05
Nodes (55): Tests for the translation stage. ``information_extraction/translator.py`` sits…, A sentence and a field label want different instructions, so they are batched…, A district repeated down a table is one string, not twenty., A half-converted date prints a year that looks Gregorian and is not., The SEE certificate prints ``certificate_title_np`` on one line and…, Without the English sibling, that value is the only one on the page., A laalpurja's ``plots`` is a list of dicts; every row must be reached., ``<field>_meta`` and ``<field>_citations`` are provenance the extractor… (+47 more)
...
Cohesion: 0.05
Nodes (63): build_data(), extract(), load_schema(), language_spec(), LanguageSpec, Target languages the translator can render a document into. One…, Everything the translator needs to know about one target language. Parameters:…, Look up a target language, naming the supported codes on failure. (+55 more)
...
### Community 16 - "AbsoluteBox"
Cohesion: 0.14
Nodes (19): _address_block(), build_citizenship(), _ea(), field_row(), multi_row(), Any, build_citizenship_old(), _ea() (+11 more)
...
### Community 19 - "LabelValue"
Cohesion: 0.26
Nodes (15): build_citizenship_back(), _ea(), field_row(), multi_row(), _officer_block(), _officer_row(), _place_block(), Any (+7 more)
...
Cohesion: 0.14
Nodes (16): Placeholder components for document furniture the render cannot reproduce. A…, Document — the root of a renderable page. doc = Document("My Certificate",…, em(), pct(), pt(), px(), Style descriptor for the HTML Document Engine. ``Style`` is an **open**…, An unrecognized CSS property name reached ``Style``. Probably a typo. (+8 more)
...
Cohesion: 0.08
Nodes (56): analyze_and_repair(), build_from_geometry(), _build_system_prompt(), _dispatch_tool(), _field_name(), generate_resources(), _image(), _load_rules() (+48 more)
...
### Community 56 - "Text"
Cohesion: 0.12
Nodes (18): build_tax_clearance(), _ea(), Any, PlaceholderBox, A labelled outline standing in for artwork that cannot be rendered. Renders a…, Inline text element rendered as a ``<span>``. Parameters: content: The text…, Text, A fixed pixel radius would not stay elliptical on a non-square box. (+10 more)
...
Cohesion: 0.18
Nodes (12): build_letter(), _ea(), Any, build_transfer_certificate(), _ea(), Any, Transfer Certificate Layout — document_builder/transfer_certificate/layout_1.py…, FlexCol (+4 more)
...
- **Why does `Style` connect `Style` to `test_components.py`, `grid.py`, `components/__init__.py`, `Document`, `translator.py`, `text.py`, `AbsoluteBox`, `Component`, `LabelValue`, `citizenship_old_back/layout.py`, `spacer.py`, `.to_css`, `html_engine/__init__.py`, `base.py`, `test_monochrome.py`, `Image`, `laalpurja/layout.py`, `Any`, `Text`, `FlexCol`?**
  _High betweenness centrality (0.240) - this node is a cross-community bridge._
- **Why does `find_violations()` connect `test_monochrome.py` to `css_surface`, `monochrome.py`, `test_autolayout.py`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `StyleWarning` connect `html_engine/__init__.py` to `translator.py`, `Style`?**
