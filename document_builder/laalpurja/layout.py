...
# CSS for contenteditable hover/focus indicators
_EDITABLE_CSS = """
...
# Devanagari digit → ASCII digit translation table for float conversion
_DEVA = str.maketrans("०१२३४५६७८९", "0123456789")

def _to_float(s) -> float:
    try:
        return float(str(s or "").translate(_DEVA))
    except ValueError:
        return 0.0

def _ea(field_name: str) -> dict[str, str]:
    """Return editable attrs dict for a data-bearing element."""
    return {"contenteditable": "true", "data-field": field_name}

def _lv(label: str, value: str, lw: str = "160px",
         field_name: str = "") -> LabelValue:
    va = _ea(field_name) if field_name else {}
    la = _ea(f"label.{field_name}") if field_name else {}
    return LabelValue(label, value, label_style=_LBL.clone(width=lw),
                      value_style=_VAL, value_attrs=va, label_attrs=la)
...
def _td(*args, field_name: str = "", **kwargs) -> TableCell:
...
def build_laalpurja(data: dict[str, Any]) -> Document:
    ...
    details_panel = Div(
        FieldGroup(
            _lv("Name of landowner:", d["owner_name"], lw="160px", field_name="owner_name"),
            addr_row,
            _lv("Husband's Name:", d["husband_name"], lw="160px", field_name="husband_name"),
            _lv("Father in Law name:", d["father_in_law"], lw="160px", field_name="father_in_law"),
            cit_date_row,
            _lv("Issued Office:", d["issue_office"], lw="130px", field_name="issue_office"),
            spacing="5px",
    ...
    def _v(plot, key):
        """Return plot field as string, converting None/null to empty string."""
        return str(plot.get(key) or "")

    for i, plot in enumerate(d["plots"]):
        total_sqm += _to_float(plot.get("area_sqm"))
        pf = f"plots.{i}."  # prefix for data-field on plot cells
        ...
        body_rows.append(TableRow(
            ...
            _td(_v(plot, "signature"),         rowspan=2, field_name=f"{pf}signature"),
        ))
