...
class LabelValue(Component):
    ...
    def to_html(self) -> str:
        ...
        label_style_attr = self.label_style.to_attr()

        # Build the label element's attribute string (style + any extra label_attrs)
        label_parts: list[str] = []
        if label_style_attr:
            label_parts.append(label_style_attr.strip())
        for key, val in self.label_attrs.items():
            label_parts.append(f'{key}="{val}"')
        label_attrs_str = (" " + " ".join(label_parts)) if label_parts else ""

        value_style_attr = self.value_style.to_attr()

        # Build extra HTML attributes for the value element
        extra_parts: list[str] = []
        if value_style_attr:
            extra_parts.append(value_style_attr.strip())
        for key, val in self.value_attrs.items():
            extra_parts.append(f'{key}="{val}"')
        value_attrs_str = (" " + " ".join(extra_parts)) if extra_parts else ""
        ...
        return (
            f"<div{attrs}>"
            f"<div{label_attrs_str}>{label_html}</div>"
            f"<div{value_attrs_str}>{value_html}</div>"
            f"</div>"
        )
